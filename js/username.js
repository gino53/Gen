const checkUsername = document.querySelectorAll('.check-username');
const submitUsername = document.querySelector('#btn-submit-username');

for (let i = 0; i < checkUsername.length; i++) {
  checkUsername[i].addEventListener('change', toggleSubmitButton);
}

function toggleSubmitButton() {
  let isChecked = false;
  for (let i = 0; i < checkUsername.length; i++) {
    if (checkUsername[i].checked) {
      isChecked = true;
      break;
    }
  }
  submitUsername.disabled = !isChecked;
}
...
formUsername.addEventListener('submit', (event) => {
  ...
  const length = parseInt(formUsername.querySelector('#length').value);
  const namesCheckbox = formUsername.querySelector('#names');
  const colorsCheckbox = formUsername.querySelector('#colors');
  const numbersCheckbox = formUsername.querySelector('#numbers');
  const charactersCheckbox = formUsername.querySelector('#characters');
  const filters = [];
